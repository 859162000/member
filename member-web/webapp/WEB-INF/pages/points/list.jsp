...
<div class="table" id="listTablePoint"><display:table
		...
		<display:column title="操作单号" class="ac">
			<c:choose>
				<c:when test="${row.pointTransCode == null }">
				调整
				</c:when>
				<c:otherwise><a href="javascript:${adk:func('doPointDataView')}(${row.id});">${row.pointTransCode }</a></c:otherwise>
			</c:choose>
