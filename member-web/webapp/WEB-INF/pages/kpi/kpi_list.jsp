...
<adk:form name="frmCreateAbatchmemKpi" action="doCreatUpLoad">
	<input type="hidden" name="status">
	<adk:func name="doCreatUpLoad" param="status" submit="yes" />
</adk:form> 
<adk:form name="frmdoKpiView" action="doView">
	<input type="hidden" name="id">
	<adk:func name="doView" param="id" submit="yes" />
</adk:form> 
<adk:form name="frmUpLoadKpiTemp"
	action="doUpLoadTemp">
	<adk:func name="doUpLoadTemp" submit="yes" />
</adk:form>
<adk:form name="frmPagingAbatchMemKpi" action="search">
	<adk:func name="search" submit="yes" />
	<input type="hidden" name="page" id="page" />
	<input type="hidden" name="sort" id="sort" />
	<input type="hidden" name="dir" id="dir" />
</adk:form>
<c:set var="importkpitemp" value="${adk:exec1(u, 'hasRight', 'member.import.kpitemp')}" />
<div>
	<table border="0">
		<tbody>
			<tr>
				...
				<button type="button" onclick="${adk:func('doCreatUpLoad')}('W');"
					class="btn upload" title="请上传2003版excle">上传KPI</button>
				<c:if test="${importkpitemp }">
...
<div class="table" id="listTableKpiFile"><display:table
		name="KPIABATCHRESULT" id="rowm" htmlId="idTable" cellspacing="0"
		cellpadding="0" style="width:100%" export="false" requestURI="">
		<display:column title="No" style="width:20px;" headerClass="ar"
			class="ar">${KPIABATCHRESULT.startIndex+rowm_rowNum}</display:column>
...
$("#listTableKpiFile").displayTagAjax({
	form : '${adk:encodens("frmPagingAbatchMemKpi")}'
});
