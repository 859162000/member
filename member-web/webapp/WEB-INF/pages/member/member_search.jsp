		...
		<tbody>
			<tr>
				...
				<input:text id="query_start" name="query_start"  readOnly="${true}" class="Wdate"
									onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',autoPickDate:true,maxDate:'#F{$dp.$D(\\\'query_end\\\')}'})" />~
				<input:text id="query_end" name="query_end" readOnly="${true}" class="Wdate"
									onclick="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',autoPickDate:true,minDate:'#F{$dp.$D(\\\'query_start\\\')}'})" />
				</td>
