...
<jsp:root version="2.1" xmlns="http://www.w3.org/1999/xhtml"
	...
	<adk:getinput var="inp" type="textarea" name="${name}" object="${object}" property="${property}" value="${value}" validate="${validate}" error="${error}" disabled="${disabled}">
		<jsp:doBody />
	</adk:getinput>

	<adk:element elm="textarea"><adk:attr name="name" value="${name}" /><adk:attr name="disabled" value="disabled" test="${inp.disabled || fdisabled}" /><adk:attr name="readOnly" value="readOnly" test="${readOnly}" /><c:forEach var="attr" items="${attrMap}"><adk:attr name="${attr.key}" value="${attr.value}" /></c:forEach>${inp.value}</adk:element>
