...
public class AbatchChangePointLet extends BaseCrudWinlet<TFileAttach> {
	...
	protected void prepareQuery(IModuleRequest req) {
		...
		refObjectType = TMemberPoint.class.getName();
	}
	...
	public int doDownLoadTemp(IModuleRequest req, IModuleResponse resp)
			throws Exception {
		uploadFile = getService(TFileAttachService.class).getFileAttach(0l, "模板", "%批量调整积分模板%", null);
		if (uploadFile != null) {
			...
			// CcsUserProfile user = (CcsUserProfile) req.getUserProfile();
				if (uploadFile != null) {
					if(uploadFile.getContentType() == null){
						...
						getService(TFileAttachService.class).createOrUpdate(uploadFile,"changeMemberPointJob",(CcsUserProfile)req.getUserProfile());
						if(uploadFile.getStatus() != null && uploadFile.getStatus().equals("W")){
							Map<String, String> params = new HashMap<String, String>();
							params.put("fileId", uploadFile.getId().toString());
							
							getService(JobScheduleService.class).scheduleJob("ChangeMemberPointJob", "member-jobs", params);
							//getService(CreateSchedulerService.class).createScheduler("changeMemberPointJob", "changeMemberPointJob", (CcsUserProfile)req.getUserProfile(), fileIdMap);
						}
					}
				}
		} catch (Exception e) {
