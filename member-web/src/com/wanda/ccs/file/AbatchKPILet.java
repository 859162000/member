...
public class AbatchKPILet extends BaseCrudWinlet<TFileAttach> {
	...
	public int doDownLoadTemp(IModuleRequest req, IModuleResponse resp)
			throws Exception {
		uploadFile = getService(TFileAttachService.class).getFileAttach(0l, "模板", "%批量导入KPI模板%", null);
		if (uploadFile != null) {
		...
		try {
			...
			if (uploadFile != null) {
				if(uploadFile.getContentType() == null){
					...
					getService(TFileAttachService.class).createOrUpdate(uploadFile,"kpiJob",(CcsUserProfile)req.getUserProfile());
					
					if(uploadFile.getStatus() != null && uploadFile.getStatus().equals("W")){
						Map<String,String> params = new HashMap<String, String>();
						params.put("fileId", uploadFile.getId().toString());
						
						getService(JobScheduleService.class).scheduleJob("KpiImportJob", "member-jobs", params);
						
						//getService(CreateSchedulerService.class).createScheduler("kpiJob", "kpiJob", (CcsUserProfile)req.getUserProfile(), fileIdMap);
					}
				}
			}
