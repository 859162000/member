...
public class UpLevel {
	static Logger logger = Logger.getLogger(UpLevel.class.getName());
...
//	}
	...
	public String  uplevels(String date){
		String ss="" ;
//		Date da = new Date();
//		String date = DateUtil.getDateStrymd(da);//时间可能是 时分秒
		...
		//得到总条数
		Long totalNum = memdao.fandMemberPointTotalNumDay(conn, date);
		...
		while (totalNum>0) {
			...
			List<MemberPoint> mplist = memdao.fandMemberPointDayPage(conn, page, date);
			//分页查今天积分有变更的会员积分表中的会员。
			for (int i = 0; i < mplist.size(); i++) {
				MemberPoint mp = mplist.get(i);
				//MemberPoint memp =	memdao.fandMemberPointByID(conn, mp.getMemberId());
				List<PointHistroy> phlist = pointDao.fandPointHistroyByLevel(conn, date, mp.getMemberId());
				if(phlist.size()!=0){
