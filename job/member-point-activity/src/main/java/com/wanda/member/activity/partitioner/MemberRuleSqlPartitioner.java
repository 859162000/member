...
public class MemberRuleSqlPartitioner implements Partitioner {
	private static final Log logger = LogFactory.getLog(MemberRuleSqlPartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		...
		try {
			...
			logger.error("Unexpected Input For Member Rule XML.", e);
		} catch (ParseException e) {
			logger.error("Parse Error For Member Rule XML.", e);
		} catch (NonTransientResourceException e) {
			logger.error("Non Transient Resource For Member Rule XML.", e);
		} catch (Exception e) {
			logger.error("Read Member Rule XML Error.", e);
		}
		...
		if(!CollectionUtils.isEmpty(ruleList)){
			for (int index = 0; index < ruleList.size(); index++) {
				...
				results.put("partition."+RuleTypeConts.MEMBER_RULE+"."+index, context);
			}
		}
