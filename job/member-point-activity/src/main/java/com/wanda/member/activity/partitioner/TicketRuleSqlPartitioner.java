...
public class TicketRuleSqlPartitioner implements Partitioner {
	private static final Log logger = LogFactory.getLog(TicketRuleSqlPartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int gridSize) {
		...
		if(!CollectionUtils.isEmpty(ruleList)){
			for (int index = 0; index < ruleList.size(); index++) {
				...
				results.put("partition."+RuleTypeConts.TICKET_RULE+"."+index, context);
			}
		}
