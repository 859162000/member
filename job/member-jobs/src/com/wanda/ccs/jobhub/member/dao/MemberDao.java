...
public class MemberDao {
	...
	/*public static void main(String[] args) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			dao.getFile("com.wanda.ccs.model.TMember", "W", 43L);
		} catch (Exception e) {
	...
	public void getFile(String refOjectType, String fileStatus, Long fileId) {
		if (refOjectType != null && fileStatus != null) {
			String sql = " SELECT FILE_ATTACH_ID ,FILE_NAME, FILE_DATA, CREATE_BY FROM t_file_attach WHERE ref_object_type = '"
					+ refOjectType
					+ "' and file_status = '"
					+ fileStatus
					+ "' and FILE_ATTACH_ID =" + fileId;
	...
	public void saveProperty(final List<String[]> strArray ,Long fileId,String fileName, String userName) throws Exception {
		...
		MemberBean succMember = null;//new MemberBean();
...
//			}else{
				...
				outer:for(int c=0,len=beans.length;c<len;c++) {
					...
					case 3:// 会员生日
						if (beans[c] != null && !"".equals(beans[c])) {
							try {
								Date da = new SimpleDateFormat("yyyy-MM-dd").parse(beans[c]);
								succMember.setBirthDate(da);
							} catch (Exception e) {
								rowDateLog(logMsg, beans, "出生日期格式有误(例：2008-08-08).请以文本格式保存", i);
							}
						} else {
					...
					case 4:// 注册影城
						if (beans[c] != null && !"".equals(beans[c])) {
							...
							if(innerCodeCache.containsKey(beans[c])) { //影城内码存在
								// 存注册影城Id
								succMember.setCinemaId(Long.valueOf(innerCodeCache.get(beans[c])));
					...
					case 5:// 是否希望被联系
						if (beans[c] != null && !"".equals(beans[c])) {
							if ("是".equals(beans[c])||beans[c].equalsIgnoreCase("YES")) {
								// 存1
								succMember.setContactable(1l);
							} else if ("否".equals(beans[c]) || beans[c].equalsIgnoreCase("NO")) {
								// 存0
...
//			}
			...
			if (logMsg.length() > 0) {
				...
				try {
					memberId = getJdbcTemplate().queryForLong("select S_T_MEMBER.NEXTVAL from dual");
					memberNum = getJdbcTemplate().queryForLong("select S_MEMBER_NUM.NEXTVAL from dual");
					...
					if(succMember.getBirthDate()!=null){
						...
						memberSqlBf.append( new SimpleDateFormat("yyyy-MM-dd").format(succMember.getBirthDate()));
					...
					String memberInfoSql = " insert into T_MEMBER_INFO (MEMBER_INFO_ID,MEMBER_ID,MANAGE_CINEMA_ID,ISDELETE,CREATE_BY,CREATE_DATE,UPDATE_BY,UPDATE_DATE,VERSION) values (S_T_MEMBER_INFO.nextval, "
							...
							+ " ,0, " + "'"+userName+"',systimestamp,'"+userName+"' ,systimestamp,0 )";
					...
					registPointAndLevel.addMemberPointByID(con, memberId, userName, 0L);
				} catch (Exception e) {
					// TODO Auto-generated catch block
