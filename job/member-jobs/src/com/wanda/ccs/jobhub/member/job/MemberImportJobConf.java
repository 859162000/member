...
public class MemberImportJobConf implements JobConfig {
	...
	public JobDetail getJobDetail() {
		if(job == null) {
			job = newJob(PathletVirtualJob.class)
		    	.usingJobData(PathletJobFactory.JOB_PATH_KEY, "/jobhub/member/MemberImportJob")
		    	.withDescription("批量导入会员信息；参数： fileId=T_FILE_ATTACH表中的主键。")
		        .withIdentity("MemberImportJob", jobGroup)
		        .build();
		}
		return job;
	}
	...
	public Trigger getTrigger(Map<String, String> triggerDataMap) {
	    ...
	    Trigger trigger = newTrigger()
	        .withIdentity("MemberImportJob-" + fileId, jobGroup)
	        .startNow() 
	        .build();
	    return trigger;
	}
