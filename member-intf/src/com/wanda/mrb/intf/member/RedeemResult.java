...
public class RedeemResult  extends ServiceBase{
	...
	protected void bizPerform() throws Exception {
		...
		//验证订单号存在
		rsq=SqlHelp.query(conn, SQLConstDef.CHECK_TRANS_ORDER, 
				this.orderNo,cinemaInnerCode,
				"1",
				String.valueOf(mid));
		rs=rsq.getResultSet();
		double exPoint = 0;
		...
		//会员状态是否被禁用
		rsq=SqlHelp.query(conn, SQLConstDef.MEMBER_CHECK_BY_MEMBERNO, memberNo);
		rs=rsq.getResultSet();
		if(rs != null && rs.next()){
		...
		SqlHelp.operate(conn, SQLConstDef.UPDATE_POINT_HISTORY,"0",String.valueOf(phid));
		SqlHelp.operate(conn, SQLConstDef.INSERT_POINT_HISTORY,
				"6",
				"1",
				String.valueOf(mid),
				"1",String.valueOf(myBalance),//变化前
				"0",
				"0",
				"0",
				String.valueOf(myBalance+Double.valueOf(exPoint)),//变化后
				"0",
				String.valueOf(exPoint),//变化额
				productName,
				orderNo,
				"2",
				"1",
		...
		SqlHelp.operate(conn, SQLConstDef.UPDATE_POINT_BALANCE, 
				String.valueOf(Double.valueOf(exPoint)),
				String.valueOf(mid));
	}
