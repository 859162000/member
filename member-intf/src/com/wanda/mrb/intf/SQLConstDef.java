...
public class SQLConstDef {
	...
	public static final String MEMBER_REGISTER_UPDATE = "UPDATE T_MEMBER " +
			...
			"UPDATE_DATE=sysdate, EMAIL=?, PHONE=?, FAV_CINEMA_ID=?, SOURCE_TYPE=?, MOBILE=? WHERE MEMBER_NO = ?";
	public static final String MEMBER_REGISTER_UPDATE_NOFAVCINEMA = "UPDATE T_MEMBER " +
			"SET NAME=?, GENDER=?, BIRTHDAY=?, STATUS=?, UPDATE_BY=?, " +
			"UPDATE_DATE=sysdate, EMAIL=?, PHONE=?, SOURCE_TYPE=?, MOBILE=? WHERE MEMBER_NO = ?";
	...
	/*根据会员id查询会员编码*/
	public static final String MEMBER_CHECK_BY_MEMBERID = "SELECT m.MEMBER_NO,m.NAME,m.MOBILE,m.EMAIL,c.SHORT_NAME," +
			...
			"to_char(m.REGIST_DATE,'yyyy-mm-dd hh24:mi:ss') AS REGIST_DATE,m.FAV_CINEMA_ID " +
