...
(function($){
	...
	var _t = {
		...
		getInnerHtml: function(popupLabel) {
			var html = '<span name="valueLabel" style="padding-right: 5px;"></span><button name="popup" type="button" class="wr_button">' + popupLabel + '</button>';
			return html;
		},
						...
						//Add criteria
			 			...
			 			grid.jqGrid({
			 				...
			 				onSelectRow: function(rowid, status, e) {
			 					if(def.singleSelect) { // 单选情况
				 					var selarrrow = $(this).jqGrid('getGridParam','selarrrow');
				 					
				 					if(status == false) {
				 						if(selarrrow && selarrrow.length == 0) { //默认选中样式
				 						...
				 						for(var i=0,len=selarrrow.length;i<len;i++) {//清除上一个选中数据
				 							if(rowid != selarrrow[i]) {
				 								$(this).setSelection(selarrrow[i]);
				 							}
				 						}
				 					}
			 					} else {
