...
//		levelChangeMapper.put("2", "'DOWN'"); 			//降级
		...
		Clause consale_date = newPlain().in("where").output("consale_date.DATE_KEY=consale.BOOK_DATE_KEY")
				...
				.depends(consale);
		//未发生卖品交易
		Clause consale_dateNot = newPlain().in("notexistsConWhere").output("consale_dateNot.DATE_KEY=consaleNot.BOOK_DATE_KEY")
		...
		Clause schedule_plan = newPlain().in("from").output(MBRODS+".T_SCHEDULE_PLAN_B schedule_plan")
				.depends(newPlain().in("where").output("schedule_plan.FILM_ID = transSales.FILM_KEY"))
				.depends(transSales);
...
//		 		.depends(transSalesAll_dateNot))
		...
		//票房交易
		//观影日期
		.add(notEmpty("watchTradeDay"), newExpression().in("where").output("transSales.SHOW_DATE_KEY", DataType.DATE)
				.depends(transSales))
		//观影日期未交易
		.add(notEmpty("watchTradeDayNot"), newExpression().in("notexistsTransWhere").output("transSalesNot.SHOW_DATE_KEY", DataType.DATE)
				.depends(transSalesNot))
		//是否节假日观影
...
//			 .depends(transSales_dateNot))
		...
		//观影消费金额
		.add(notEmpty("purchaseMoney"), newExpression().in("having").output("sum(transSales.BK_ADMISSIONS)-sum(transSales.BK_ADMISSIONS)", DataType.INTEGER)
