...
//		combineSegmentDo.setVersion(segmentVo.getVersion());
	...
	public ResponseMessage updateSegments() throws Exception {
		ObjectMapper mapper = new ObjectMapper();
		...
		if(segmentId != null) {
			...
			for(int i=0,len=ids.length;i<len;i++) {
				...
				if(subVo == null) {
					...
					subVo.setSegmentVersino(segmentVo.getVersion());	//对应的客群版本
			...
			try {
				//segmentService.updateStartCalCount(segment.getSegmentId());
				return new VersionResponseMessage(ResponseLevel.INFO, "客群保存成功！", vo.getVersion());
			}
			catch(Throwable t) {
				log.error("Start calculate count error! segmentId=" + this.segmentId, t);
				return new VersionResponseMessage(ResponseLevel.WARNING, "客群已保存成功，但计算客群任务未能正确执行！请与系统管理员联。", vo.getVersion());
			}
			
		} else {
			return new ResponseMessage(ResponseLevel.ERROR, "保存客群失败！");
		}
	}
