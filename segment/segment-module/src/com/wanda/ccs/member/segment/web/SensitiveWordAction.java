...
import com.wanda.ccs.member.segment.web.SegmentAction.SegmentDo;
...
private static Log log = LogFactory.getLog(SensitiveWordAction.class);
	...
	public JqgridQueryResult<Map<String, Object>> query() throws Exception {
		...
		QueryResultVo<Map<String, Object>> result = sensitiveService.queryList(converter.convertParam(queryParam), criterionList, userProfile);
	...
	public SegmentVo getCount() throws Exception {
		return segmentService.getCount(this.wordId);
	}

	

	public ResponseMessage insert() throws Exception {
		SensitiveDo sensitive = JsonCriteriaHelper.parseSimple(json, SensitiveDo.class);
		...
		//segment.convertAllowModifier();
			...
			try {
				return new VersionResponseMessage(ResponseLevel.INFO, "敏感词保存成功！", sensitive.getVersion());
			}
			catch(Throwable t) {
				log.error("Start calculate count error! wordId=" + this.wordId, t);
				return new VersionResponseMessage(ResponseLevel.WARNING, "敏感词保存失败。", sensitive.getVersion());
			}
	...
	public ResponseMessage update() throws Exception {
		SensitiveDo sensitive = JsonCriteriaHelper.parseSimple(json, SensitiveDo.class);
			SegmentVo old = segmentService.get(sensitive.getWordId());
			sensitiveService.update(sensitive);
			return new VersionResponseMessage(ResponseLevel.INFO, "客群更新成功！", sensitive.getVersion());
	}
	...
	public CriteriaQueryResult getCriteriaResult() throws Exception {
		SensitiveWordVo vo = sensitiveService.get(wordId);
		CriteriaQueryResult result = sensitiveService.getSegmentQuery(vo.getWordId());
		return result;
	}
	/**
	 * 用于和界面交互时的客群对象
