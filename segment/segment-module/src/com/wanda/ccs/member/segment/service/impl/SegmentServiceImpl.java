...
public class SegmentServiceImpl implements SegmentService {
	...
	public QueryResultVo<Map<String, Object>> queryList (
		...
		CriteriaParser listParser = newParser(SELECT_PARAGRAPHS)
				.add(newPlain().in("select").output("s.SEGMENT_ID, s.CODE, s.NAME, s.CAL_COUNT, s.STATUS, s.COMBINE_SEGMENT, s.CREATE_BY, s.UPDATE_BY, s.UPDATE_DATE, s.ALLOW_MODIFIER"))
				.add(newPlain().in("where").output("s.ISDELETE='0'"))
				.add(segmentTable)
				.add(clauseMap);
	...
	public void updateStartCalCount(Long segmentId) {
		//设置客群状态为正在计算状态
		int rows = getJdbcTemplate()
			.update("update T_SEGMENT t set t.STATUS=? where t.SEGMENT_ID=?",
				new Object[] {SegmentVo.STATUS_CALCULATING, segmentId});
	...
	public SegmentVo getCount(Long segmentId) {
		SegmentVo segment = getJdbcTemplate().queryForObject(
				"select t.SEGMENT_ID, t.CAL_COUNT, t.CAL_COUNT_TIME, t.STATUS from T_SEGMENT t where t.SEGMENT_ID= ?",
				new Object[]{segmentId},
	...
	public List<CombineSegmentSubVo> getSelectedSegment(Long[] segmentIds) {
		...
		List<CombineSegmentSubVo> list = getJdbcTemplate().query(sql.toString(), segmentIds, new RowMapper<CombineSegmentSubVo>() {
			...
			public CombineSegmentSubVo mapRow(ResultSet rs, int rowNum)
				...
				subVo.setSegmentVersino(rs.getLong("VERSION"));
		...
		sub = getJdbcTemplate().query(sql, new Object[] { segmentId }, new ResultSetExtractor<List<CombineSegmentSubVo>>() {
			...
			public List<CombineSegmentSubVo> extractData(ResultSet rs)
				...
				while (rs.next()) {
					...
					combineSegmentSubVo.setSegmentVersino(rs.getLong("SEGMENT_VERSION"));
	...
	public void insertCombineSegment(SegmentVo segment, List<CombineSegmentSubVo> sub) {
		...
		// 保存子客群
		String insertSql = "insert into T_COMBINE_SEGMENT_SUB(combine_segment_sub_id,segment_id,sub_segment_id,sort_index,set_relation,cal_count,cal_count_time,segment_version,create_by,create_date,update_by,update_date,version) values";
	...
	public List<Map<String, Object>> getCountAlter(Long segmentId) {
		String sql = "select T.SEGMENT_ID,T.CODE,T.CAL_COUNT,T.CAL_COUNT_TIME,T.STATUS,S.SUB_SEGMENT_ID,S.COUNT_ALTER,TT.CAL_COUNT as CAL_COUNT_SUB,TT.CAL_COUNT_TIME as CAL_COUNT_TIME_SUB,TT.STATUS as STATUS_SUB from T_SEGMENT T, T_COMBINE_SEGMENT_SUB S, T_SEGMENT TT WHERE T.SEGMENT_ID=S.SEGMENT_ID AND S.SUB_SEGMENT_ID=TT.SEGMENT_ID AND T.SEGMENT_ID=?";
