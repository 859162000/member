...
$(function() {
	...
	var getSegments = context+'/segment/SegmentAction/getSegments.do';
	var getSelectedSegment = context+'/segment/SegmentAction/getSelectedSegments.do';
	var calCountUrl = context+'/segment/SegmentAction/calCount.do';
	var insertUrl = context+'/segment/SegmentAction/insertSegments.do';
	var updateUrl = context+'/segment/SegmentAction/updateSegments.do';
	var updateCountAlter = context+'/segment/SegmentAction/updateCountAlter.do';
	...
	function addSegment(value, index, type) {
		...
		if(parseInt(value.countAlter) < 0) {
			...
			$("td[name=countalter]", row).html(value.countAlter);
		}
		...
		if(selectedSegment.length == 0) {	//主客群记录没有操作类型
			...
			if(type=='edit') {// 编辑时回显
				row.find("select").val(value.setRelation);	//设置选中项
				if(value.setRelation == 'UNION') {
					row.find('img').attr('src', '../images/set-union.png');
				} else if(value.setRelation == 'INTERSECT') {
					row.find('img').attr('src', '../images/set-intersect.png');
				} else if(value.setRelation == 'MINUS') {
					row.find('img').attr('src', '../images/set-minus.png');
				}
			} else if(type == 'view') {
				$('select', row).remove();
				if(value.setRelation == 'UNION') {
					$('td[name=type]', row).html('合集&nbsp;&nbsp;<img src="../images/set-union.png" height="25" width="40">');
				} else if(value.setRelation == 'INTERSECT') {
					$('td[name=type]', row).html('交集&nbsp;&nbsp;<img src="../images/set-intersect.png" height="25" width="40">');
				} else if(value.setRelation == 'MINUS') {
					$('td[name=type]', row).html('差集&nbsp;&nbsp;<img src="../images/set-minus.png" height="25" width="40">');
				}
			}
		}
	...
	function renderBtn(scope) {
		...
		$("select", scope).live("change", function() {
			...
			if($(this).val() == 'UNION') {
				$(this).next('img').attr('src', '../images/set-union.png');
			} else if($(this).val() == 'INTERSECT') {
				$(this).next('img').attr('src', '../images/set-intersect.png');
			} else if($(this).val() == 'MINUS') {
				$(this).next('img').attr('src', '../images/set-minus.png');
			}
			// 设置选中项
	...
	function getValidatedData() {
		...
		if(rows && rows.length > 0) {
			...
			schemeData = '{"ids":"'+ids+'","types":"'+types+'","conTypes":"'+conTypes+'","segmentId":"'+voSegmentId+'","name":"'+voData+'","version":"'+voVersion+'"}';
		}
	...
	function countAlterCheck(segmentId) {
		...
		countAlterTimer = setInterval(function() {
			$.ajax({
					...
					if(result.level == 'ERROR') {	//复合客群计算失败
						...
						if(isStop == false) {
							...
							if(obj[0].STATUS == '40') {	//失败
								...
								$('[wrType]', inputCombineForm).wrender('setValue', {code: obj[0].CODE,calCount: obj[0].CAL_COUNT,calCountTime: obj[0].CAL_COUNT_TIME});
								$.msgBox('info', '', '复合客群计算完成！');
							}
						}
					}
				}
	...
	combineAction.openDialog = function(segmentId, type) {
		...
		if(segmentId != null) {
			//将复合客群名称编辑框定义为不可编辑
			$('input[name=name]', inputCombineForm).attr('disabled','disabled');
			isNew = false;
			$.ajax({
				url: getSegments,
				data: 'segmentId=' + segmentId,
