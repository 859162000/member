...
$(function() {
		...
		buttons: [
            {text:'下一步', click: function() { 
            	...
            	if($('input[name=combineSegment]:checked', selectInputDialog).val() == 'true') {
            		combineAction.openDialog(null);//打开复合客群窗口
            	}
            	else {
            		onNewCommon();
            	}
	...
	function onNewCommon() {
		...
		sortNameChange('00');
		setCalCount(0, '', '20', false); //因为新建后自动执行后台计算，这里隐藏计算数量行, 状态被设为'20'表示‘计算中’
	...
	$(resultList).on('click', 'button[name=onModifyBtn]', function (){
		...
		var TYPE = resultList.jqGrid('getCell', rowid, 'COMBINE_SEGMENT');
        ...
        if(TYPE == '普通'){
        	$.ajax({
    			...
    			success: function(result) {
    				...
    				setCalCount(result.calCount, result.calCountTime, result.status, true);
        	...
        	combineAction.openDialog(segmentId);
        }
	});
	...
	$('#onCalCountBtn', inputForm).click(function() {
		...
		var formVal = $('input[name="segmentId"],input[name="status"],span[name=calCount],div[name=calCountTime]', inputForm).wrender('getValue');
		
		calCountDialog.dialog('open');
		setCalCount(0, '', '20', true);//设置正在计算中状态
		...
		$.ajax({
			...
			success: function(result) {
				if(result.level == 'INFO') {
					...
					//发起客群计算任务失败
					setCalCount(formVal.calCount, formVal.calCountTime, formVal.status, true);//设置正在计算中状态
	...
	function setCalCount(count, calCountTime, status, showRow) {
		var calCountRow = $('#calCountRow');
		
		$('#calCountRow input[name="status"]').wrender('setValue', {'status': status});
		var calCountElm = $('span[name=calCount]', calCountRow);
		var calCountTimeElm = $('div[name=calCountTime]', calCountRow);
