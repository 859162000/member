(function($){
	...
	$.wrender.addComposite({
	    ...
	    columns: [ //查询结果列
	        ...
	        {name:'CAL_COUNT', label:'执行状态', width:120, sortable:false},
	        {name:'STATUS', label:'波次结束时间', width:120, sortable:false},
	...
	$.wrender.addComposite({
	    ...
	    columns: [ 
	        ...
	        {name:'FILM_NAME', label:'影片名称', width:90, sortable:true},
	        {name:'SHOW_SET', label:'放映制式', width:90, sortable:false},
	        {name:'FILM_CATE', label:'类别', width:40, sortable:false},
	        {name:'FILM_TYPES', label:'影片类型', width:90, sortable:false}, //T_FT_TYPE
	        {name:'COUNTRY', label:'国家', width:40, sortable:false},
			{name:'DIRECTOR', label:'导演', width:80, sortable:false},
			{name:'MAIN_ACTORS', label:'演员', width:170, sortable:false},
			{name:'PREMIERE_DATE', label:'首映日期', width:90, sortable:true},
			{name:'END_DATE', label:'落幕日期', width:90, sortable:false}
		]
	});
